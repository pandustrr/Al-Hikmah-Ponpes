        ...
        <PublicLayout title={lembaga.nama} navTheme="dark" isLembaga={true}>
            ...
            <div className="bg-brand-primary py-24 text-center">
                <div className="max-w-7xl mx-auto px-4">
                    <h1 className="text-3xl md:text-5xl font-semibold text-white tracking-tighter mb-4 uppercase">{lembaga.nama}</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-24 space-y-12">
                            <section id="tentang">
                                <h2 className="text-xs font-semibold uppercase tracking-widest text-brand-primary mb-4">Tentang Kami</h2>
                            ...
                            <section className="bg-brand-secondary p-8 border border-brand-light rounded-[0.25rem]">
                                <h2 className="text-xs font-semibold uppercase tracking-widest text-brand-primary mb-6">Visi & Misi</h2>
                                <div className="space-y-6">
                                    <div>
                                        <h3 className="font-semibold text-brand-primary mb-2">Visi</h3>
                                        <p className="text-sm text-brand-accent">{lembaga.visi || 'Visi lembaga belum diatur.'}</p>
                                    </div>
                                    <div>
                                        <h3 className="font-semibold text-brand-primary mb-2">Misi</h3>
                    ...
                    <div className="lg:col-span-2 space-y-24">
                        ...
                        <section id="berita">
                            <div className="flex justify-between items-center mb-10 pb-4 border-b-2 border-brand-primary">
                                <h2 className="text-2xl font-semibold text-brand-primary uppercase tracking-tighter">Prestasi {lembaga.slug.toUpperCase()}</h2>
                                <span className="text-xs font-semibold bg-brand-primary text-white px-3 py-1 uppercase tracking-widest">{prestasi.length} Items</span>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {prestasi.map(p => (
                                    <div key={p.id} className="group">
                                        ...
                                        <h3 className="font-semibold text-brand-primary group-hover:text-brand-primary transition-colors leading-tight">{p.judul}</h3>
                                    </div>
                                ))}
                            </div>
                        </section>
                        ...
                        <section id="fasilitas">
                            <div className="flex justify-between items-center mb-10 pb-4 border-b-2 border-brand-primary">
                                <h2 className="text-2xl font-semibold text-brand-primary uppercase tracking-tighter">Kegiatan Santri</h2>
                                <span className="text-xs font-semibold bg-brand-primary text-white px-3 py-1 uppercase tracking-widest">{kegiatan.length} Items</span>
                            </div>
                            <div className="space-y-8">
                                {kegiatan.map(k => (
                                    <div key={k.id} className="flex flex-col md:flex-row gap-8 items-center bg-white p-6 border border-brand-light hover:border-brand-primary/30 transition-all rounded-[0.25rem] group">
                                        ...
                                        <div>
                                            <div className="text-xs font-semibold text-brand-primary mb-2">{new Date(k.tanggal).toLocaleDateString('id-ID')}</div>
                                            <h3 className="text-xl font-semibold text-brand-primary mb-3 group-hover:text-brand-primary transition-colors">{k.judul}</h3>
