        ...
        <section className="py-12 md:py-24 bg-white border-b border-brand-light reveal-section">
            <div className="max-w-7xl mx-auto px-4">
                <div className="text-center mb-10 md:mb-20 reveal-element-up">
                    <h2 className="text-xs font-bold text-brand-accent uppercase tracking-[0.25em] mb-3">Informasi Terkini</h2>
                    <h3 className="text-3xl md:text-4xl font-bold text-brand-primary tracking-tighter uppercase mb-5">Update Lembaga</h3>
                                ...
                                <div key={lembaga.id} className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 items-start border-l-4 border-brand-secondary pl-5 md:pl-8 reveal-element-up">
                                    <div className="lg:col-span-3">
                                        <div className="sticky top-24">
                                            <h3 className="text-6xl font-bold text-brand-primary/5 mb-[-1.2rem] uppercase">{lembaga.slug}</h3>
                                            <h4 className="text-xl font-bold text-brand-primary mb-4 uppercase tracking-tighter">{lembaga.nama}</h4>
                                            <Link href={`/${lembaga.slug}`} className="text-brand-accent text-xs font-bold hover:text-brand-primary uppercase tracking-[0.1em] inline-flex items-center bg-brand-secondary px-4 py-2.5 transition-all rounded-[0.25rem] group">
                                    ...
                                    <div className="lg:col-span-9">
                                        ...
                                        {hasBerita ? (
                                            <div className="space-y-5">
                                                <h5 className="text-xs font-bold uppercase tracking-widest text-brand-primary border-b-2 border-brand-primary pb-2 flex items-center gap-2">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                                    {beritaItems.map(b => (
                                                        <Link key={b.id} href={`/berita/${b.slug}`} className="group bg-white border border-brand-secondary hover:border-brand-primary transition-all rounded-[0.25rem] overflow-hidden block">
                                                             ...
                                                             <div className="p-3">
                                                                 <div className="text-[10px] font-bold text-brand-accent uppercase tracking-widest mb-1 opacity-70">
                                                                     {b.category?.name || 'Berita'}
                                                                 </div>
                                                                 <h6 className="font-bold text-brand-primary group-hover:text-brand-accent transition-colors leading-snug text-xs sm:text-sm line-clamp-2">{b.judul}</h6>
                                                             </div>
                                                         </Link>
                                                    ))}
                                                </div>
                                                <Link href={`/berita`} className="inline-flex items-center gap-2 text-xs font-bold text-brand-accent hover:text-brand-primary uppercase tracking-[0.1em] transition-all">
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                                    {prestasiItems.map(p => (
                                                        <div key={p.id} className="group bg-white border border-brand-secondary hover:border-brand-primary transition-all rounded-[0.25rem] overflow-hidden">
                                                            ...
                                                            <div className="p-3">
                                                                <h6 className="font-bold text-brand-primary group-hover:text-brand-accent transition-colors leading-snug text-xs sm:text-sm line-clamp-2">{p.judul}</h6>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
