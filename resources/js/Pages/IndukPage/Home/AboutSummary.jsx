                    ...
                    <div className="lg:col-span-9 space-y-24">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-9 gap-8 lg:gap-12 items-start">
                            ...
                            <div className="lg:col-span-5 space-y-6">
                                ...
                                <div className="flex flex-wrap gap-4 pt-2">
                                    <Link href="/pendaftaran" className="btn-primary px-8 py-3 text-[10px] font-black uppercase tracking-[0.2em] shadow-lg">Pendaftaran Siswa Baru</Link>
                                    <Link href="/profil" className="border-2 border-brand-primary text-brand-primary px-8 py-3 text-[10px] font-black uppercase tracking-[0.2em] hover:bg-brand-primary hover:text-white transition-all rounded-[0.25rem]">Profil Lengkap</Link>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-12">
                            <div className="text-center lg:text-left">
                                ...
                                <h3 className="text-3xl font-black text-brand-primary tracking-tighter uppercase mb-5">Lembaga Pendidikan</h3>
                            ...
                            <div className="grid grid-cols-2 gap-3 md:gap-6">
                                {lembagas.map((lembaga) => (
                                    ...
                                    >
                                        <div className="text-[8px] md:text-[10px] font-bold text-brand-accent uppercase tracking-widest mb-2 md:mb-3">Pendidikan Formal</div>
                                        <h4 className="text-sm md:text-2xl font-black text-brand-primary mb-3 md:mb-5 group-hover:text-brand-accent transition-colors uppercase leading-tight">{lembaga.nama}</h4>
                                        <p className="hidden md:block text-brand-accent text-sm mb-8 leading-relaxed line-clamp-2">
                                            {lembaga.deskripsi}
                                        </p>
                                        <div className="flex items-center text-brand-primary font-black text-[8px] md:text-[10px] uppercase tracking-widest border-t border-brand-secondary pt-3 md:pt-5">
                        ...
                        <div className="sticky top-28 space-y-12">
                            ...
                            <div>
                                <div className="border-b-2 border-brand-primary pb-3 mb-6">
                                    <h4 className="text-[10px] font-black text-brand-primary uppercase tracking-[0.2em] flex items-center justify-between">
                                        Pengumuman
                                ...
                                <div className="space-y-6">
                                    {announcements.map((item, i) => (
                                        <Link key={i} href="/berita?kategori=pengumuman" className="block group border-b border-brand-secondary pb-4 last:border-0 hover:pl-2 transition-all duration-300">
                                            <div className="text-[8px] font-black text-brand-accent uppercase tracking-widest mb-1 opacity-70">Berita · {item.date}</div>
                                            <p className="text-[11px] font-bold text-brand-primary group-hover:text-brand-accent transition-colors leading-snug line-clamp-2">{item.title}</p>
                                        </Link>
                                    ))}
                                    <div className="pt-2">
                                        <Link href="/berita?kategori=pengumuman" className="text-[10px] font-black text-brand-accent hover:text-brand-primary uppercase tracking-[0.2em] transition-all flex flex-col group/link">
                                ...
                                <div className="space-y-6">
                                    {articles.map((item, i) => (
                                        <Link key={i} href="/berita?kategori=artikel" className="block group border-b border-brand-secondary pb-4 last:border-0 hover:pl-2 transition-all duration-300">
                                            <div className="text-[8px] font-black text-brand-accent uppercase tracking-widest mb-1 opacity-70">Artikel · {item.date}</div>
                                            <p className="text-[11px] font-bold text-brand-primary group-hover:text-brand-accent transition-colors leading-snug line-clamp-2">{item.title}</p>
                                        </Link>
                                    ))}
                                    <div className="pt-2">
                                        <Link href="/berita?kategori=artikel" className="text-[10px] font-black text-brand-accent hover:text-brand-primary uppercase tracking-[0.2em] transition-all flex flex-col group/link">
                                            <span>Lihat Semua</span>
                                            <span className="mt-1 group-hover/link:translate-x-2 transition-transform duration-300">→</span>
                                        </Link>
                                    </div>
